...
#category2~로 시작되는것은 위에 의료,문화등으로 카테고리 해준것을 각각 데이터 셋으로 나누고, 월별 결제금액를 나타내준것
##########################################################################################
...
jhu_cases<-jhu_cases[,-length(colnames(jhu_cases))]#가장 최신 일자의 각 나라별 확진자수(누적)
jhu_cases1<-jhu_cases1[-(1:(length(jhu_cases1$Afghanistan_cases)-2)),]
jhu_cases1<-jhu_cases1[-2,]#두번째로 최신일자의 각 나라별 확진자수(누적)
...
colnames(jhu_cases)<-c("confirmed","country")
colnames(jhu_cases1)<-c("confirmed")
...
jhu_deaths<-jhu_deaths[,-length(colnames(jhu_deaths))]##가장 최신 일자의 각 나라별 사망자수(누적)
jhu_deaths1<-jhu_deaths1[-(1:(length(jhu_deaths1$Afghanistan_death)-2)),]
jhu_deaths1<-jhu_deaths1[-2,]#두번째로 최신일자의 각 나라별 사망자수(누적)
...
colnames(jhu_deaths)<-c("deaths","country")
colnames(jhu_deaths1)<-c("deaths")
...
jhu_rec<-jhu_rec[,-length(colnames(jhu_rec))]##가장 최신 일자의 각 나라별 회복자수(누적)
jhu_rec1<-jhu_rec1[-(1:(length(jhu_rec1$Afghanistan_rec)-2)),]
jhu_rec1<-jhu_rec1[-2,]#두번째로 최신일자의 각 나라별 회복자수(누적)
...
colnames(jhu_rec)<-c("rec","country")#해당일에 회복된 사람들의 수
...
basemap = leaflet(plot_map) %>% 
    ...
    addCircles(data = cv_cases, lat = ~ latitude, lng = ~ longitude, weight = 1, radius = ~(death)^(1/4)*3.5e4*penalty,
    ...
    addCircles(data = cv_cases, lat = ~ latitude, lng = ~ longitude, weight = 1, radius = ~(rec)^(1/4)*3.5e4*penalty,
